...
from .utils import FreeMatchThresholingHook, resizemix, one_hot, ce_loss
...
class FreeMatch(AlgorithmBase):
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            ...
            # calculate mask
            mask, valid_indices = self.call_hook("masking", "MaskingHook", logits_x_ulb=logits_x_ulb_w)
            ...
            conf_labels = torch.cat(
                          [one_hot(y_lb, self.num_classes, self.args.gpu), one_hot(pseudo_label[valid_indices], self.num_classes, self.args.gpu)], dim=0)
            conf_data = torch.cat([x_lb, x_ulb_s[valid_indices]])
            
            mixed_x, mixed_y, _ = resizemix(conf_data, conf_labels, self.args.gpu, alpha=1.0)

            logits_mix = self.model(mixed_x)
            logits_mix_cert = logits_mix['logits']
            ...
            if mask.sum() > 0:
               ...
               ent_loss = 0.0
            # ent_loss = 0.0
            total_loss = sup_loss + self.lambda_u * unsup_loss + self.lambda_e * ent_loss + mix_loss
