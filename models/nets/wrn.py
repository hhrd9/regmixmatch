...
class WideResNet(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, is_remix=False, is_full=False):
        ...
        if self.is_full:
            self.fc1 = nn.Linear(channels[3]*64, channels[3])
    ...
    def forward(self, x, only_fc=False, ood_test=False):
        if only_fc:
            return self.fc(x)
        out = self.extract(x)
        if self.is_full:
            flattened_output = out.view(out.size(0), -1)
...
class build_WideResNet:
    ...
    def build(self, num_classes, is_full):
        return WideResNet(
