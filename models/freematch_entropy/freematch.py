...
class FreeMatch:
    def __init__(self, net_builder, num_classes, num_labels, ema_m, lambda_u, lambda_e, \
        ...
        if self.num_labels/self.num_classes == 400:
            self.model = net_builder(num_classes=num_classes, is_full=True)
        else:
            self.model = net_builder(num_classes=num_classes, is_full=False)
        self.ema_model = None
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                ...
                if args.num_labels/args.num_classes >= 100:
                    ...
                    mixed_x, mixed_y, _ = resizemix(x_ulb_s[valid_indices], 
                ...
                srm_loss = self.lambda_u * unsup_loss + mix_loss
                total_loss = sup_loss + self.lambda_e * ent_loss + srm_loss + cam_loss
